// グローバル変数
        ...
        async function loadAllData() {
            try {
                ...
                const dataPromises = categories.map(async (category) => {
                    try {
                        console.log(`${category.filename} 読み込み開始...`);
                        ...
                        console.log(`${category.filename} 内容:`, dataText.substring(0, 50));
        ...
        function createListRow(item) {
            ...
            row.innerHTML = `
                ...
                <td>車で${item.accessibility.hanedaIC}分</td>
                <td>${item.revenue}</td>
                <td>
        ...
        function applyFilters() {
            ...
            filteredData = locationData.filter(item => {
                ...
                if (distanceFilter) {
                    ...
                    if (item.accessibility.hanedaIC < min || 
                        (max !== Infinity && item.accessibility.hanedaIC >= max)) return false;
                }

                // キーワード検索
                if (searchText) {
                    const searchableText = `${item.name} ${item.address} ${item.category} ${item.categoryName || ''} ${item.notes || ''}`.toLowerCase();
        ...
        // 詳細表示（修正版：アクセス・交通セクション削除、補足情報追加）
            ...
            document.getElementById('modalBody').innerHTML = `
                <div class="detail-grid">
                    ...
                    <div class="detail-section" style="grid-column: 1 / -1;">
                        <h3>補足情報</h3>
                        <div class="info-item">
                            <div class="info-value" style="line-height: 1.6; white-space: pre-wrap;">${item.notes}</div>
                        </div>
                    </div>
                    ` : ''}
                </div>
        ...
        // CSV変換（修正版：花田ICまでの距離、補足情報追加）
        function convertToCSV(data) {
            const headers = [
                'カテゴリ', '名称', '種別', 'カテゴリ詳細', '住所', '面積(m²)', '区分',
                '花田ICまで(分)', '山陽姫路ICまで(分)', '収益性',
                '上下水道', '電力', 'ガス', '通信', '補足情報'
            ];
